...
(defn branch [weight]
  (let [answer (map (fn [x] (conj weight x)) (vals (to-weight)))]
    answer)
  )
...
(if (some #(= '(113 147 71 129) %) answer) (println "success") (println "failure"))
...
(def mn 26)
(def a (branch-bound mtest (list ()) mn))
(println a)

;(def answer (leader-board mtest mn))
;(pretty-print-weights answer)
...
(comment
  (def a (branch-bound stest (list ()) sn))
  (println a)
  (def b (branch-bound stest a sn))
  (println b)


  (def c (branch-bound stest b sn))
  (println c)
  (def d (branch-bound stest c sn))
  (println d)
  (def e (branch-bound stest d sn))
  (println e)
