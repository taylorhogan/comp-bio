...
; determin if the peptide mass = parenet spectrum mass
...
(defn score [candidate spectrum]
  (let [all (all-k-clumps-cyclic candidate)]
...
(defn branch [weight]
  (map (fn [x] (concat x weight)) vals (to-weight))
  )
...
(defn branch-all [weights]
  (loop [iset weights
    ...
    (if (empty? iset) out1-set
      (recur (rest iset) (into out1-set (branch (first iset)))))
    )
  )

; one step of branch/bound
(defn branch-bound [original horizon NUM]
  (leader-filter (branch-all horizon) original NUM)
  )
...
(defn leader-board [weights NUM]
  ...
  (loop
    ...
    (if (or (= index 1000) (empty? last)) best
      (do
        ;(println "step" index last)
        (recur last (branch-bound weights last NUM) (inc index))
        )
      )
    )
...
;113-147-71-129
(def sn 10)
(def answer (leader-board stest sn))
...
;156-71-113-114-131-156-113-101-129-128-128-114-128-103-97-131-131-113-131-113-128-115-128-113
